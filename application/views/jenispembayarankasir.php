		...
		<div class="col-lg-12">
			<div class="card">
				...
				<div class="card-body table-responsive">
					<table class="table table-striped table-bordered table-hover" id="tabel-jenispembayaran" width="100%" style="font-size:120%;">
						<thead>
							<tr>
								<th>No</th>
...
<!-- Bootstrap modal -->
<div class="modal fade" id="modal_jenispembayaran" role="dialog">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h3 class="modal-title"><i class="glyphicon glyphicon-info"></i> Form Pelanggan</h3>
			</div>
			<form role="form  col-lg-6" name="Pelanggan" id="frm_jenispembayaran">
				<div class="modal-body form">
					<div class="row">
						<input type="hidden" id="jpbc_id" name="jpbc_id" value="">

						<div class="col-lg-12">
							<div class="form-group">
								<label>Jenis Pembayaran</label>
								<input type="text" class="form-control" name="jpbc_nama" id="jpbc_nama" placeholder="" required>
							</div>
						</div>
...
<script>
	...
	function drawTable() {
		$('#tabel-jenispembayaran').DataTable({
			"destroy": true,
			dom: 'Bfrtip',
			lengthMenu: [
				[10, 25, 50, -1],
	...
	function log_tambah() {
		reset_form();
		$("#jpbc_id").val(0);
		$("frm_jenispembayaran").trigger("reset");
		$('#modal_jenispembayaran').modal({
			show: true,
			keyboard: false,
			backdrop: 'static'
		});
	...
	$("#frm_jenispembayaran").submit(function(e) {
		e.preventDefault();
		...
		$.ajax({
			...
			success: function(d) {
				...
				if (res.status == 1) {
					...
					drawTable();
					reset_form();
					$("#modal_jenispembayaran").modal("hide");
				} else {
	...
	function ubah_jenis_pembayaran(id) {
		event.preventDefault();
		$.ajax({
			type: "POST",
			url: "cari",
