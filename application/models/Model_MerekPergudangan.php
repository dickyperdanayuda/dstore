<?php
  class Model_MerekPergudangan extends CI_Model {
  
  	var $table = 'gdgs_merek';
	var $column_order = array('mrkg_nama'); //set column field database for datatable orderable
	var $column_search = array('mrkg_nama'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('mrkg_nama' => 'asc'); // default order  	private $db_sts;
	...
	public function cari_merek($id)
	{	$this->db->from('gdgs_merek');
		$this->db->where("mrkg_id", $id);
		$query = $this->db->get();
