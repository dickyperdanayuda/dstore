<?php
  class Model_PembelianPergudangan extends CI_Model {
  
  	var $table = 'gdgs_pembelian_produk';
	var $column_order = array('pblg_harga_beli'); //set column field database for datatable orderable
	var $column_search = array('pblg_harga_beli', 'pblg_status', 'prdg_nama'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('pblg_tgl' => 'desc'); // default order  	private $db_sts;
	...
	private function _get_datatables_query()
	{
		...
		$this->db->join('gdgs_produk', 'prdg_id = pblg_prd_id', 'left');
		$i = 0;
	...
	{	$this->db->from("gdgs_stok");
		$this->db->where("stk_pblg_id", $id);
		$query = $this->db->get();
