<?php
  class Model_ProdukPergudangan extends CI_Model {
  
  	var $table = 'gdgs_produk';
	var $column_order = array('prdg_nama'); //set column field database for datatable orderable
	var $column_search = array('prdg_nama', 'prdg_ktg_id', 'prdg_deskripsi'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('prdg_nama' => 'asc'); // default order  	private $db_sts;
	...
	{	$this->db->from('gdgs_produk');
        $this->db->join('gdgs_kategori','prdg_ktg_id = ktg_id','left');
        $this->db->join('gdgs_merek','prdg_mrk_id = mrkg_id','left');
		$this->db->where("prdg_id", $id);
		$query = $this->db->get();
